	...
	R: Read + Seek,
{
	fn frame(&mut self) -> error::Result<Buffer<P, C, Vec<C>>> {
		let image = tga::read(self.inner.by_ref(), ColFmt::Auto)?;

		macro_rules! buffer {
			($ch:ty, $ty:path) => {{
				Ok(cast::Into::<P, C>::into(Buffer::<$ty, $ch, _>::from_raw(
					image.w as u32,
					image.h as u32,
					image.buf,
				)
		...
		match image.fmt {
			ColFmt::Y => buffer!(u8, color::Luma),
			ColFmt::YA => buffer!(u8, color::Lumaa),
			ColFmt::RGB => buffer!(u8, color::Rgb),
			ColFmt::RGBA => buffer!(u8, color::Rgba),
